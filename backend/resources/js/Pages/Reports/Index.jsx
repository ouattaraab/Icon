    ...
    return (
        <DashboardLayout title="Rapports & Statistiques">
            ...
            <div style={{
                ...
                <span style={{ color: t.textMuted, fontSize: '0.8rem' }}>P\u00e9riode :</span>
                <input
            ...
            {dailyEvents?.length > 0 && (
                <div style={{ ...cardStyle, marginBottom: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.5rem' }}>
                        <span style={{ color: t.textFaint, fontSize: '0.65rem' }}>
                    ...
                    {platformUsage?.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            {platformUsage.map((item) => {
                                ...
                                const color = platformColors[item.platform?.toLowerCase()] || t.textFaint;
                                return (
                    ...
                    {eventTypes?.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            {eventTypes.map((item) => {
                                ...
                                const color = eventTypeColors[item.event_type] || t.textFaint;
                                return (
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{
                                                width: `${pct}%`,
                                                minWidth: pct > 0 ? 8 : 0,
                                                background: severityColors[item.severity] || t.textFaint,
                                ...
                                {severityDistribution.map((item) => (
                                    <div key={item.severity} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                            ...
                                            <span style={{ color: t.textSecondary, fontSize: '0.8rem', textTransform: 'capitalize' }}>
                                                {item.severity}
                                            </span>
                                        </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tr style={{ borderBottom: `1px solid ${t.border}` }}>
                                {['D\u00e9partement', 'Machines', '\u00c9v\u00e9nements', 'Blocages', 'Taux blocage'].map((h) => (
                                    <th key={h} style={{
                                        ...
                                        color: t.textMuted, fontSize: '0.65rem',
                        ...
                        <tbody>
                            {departmentStats.map((dept) => {
                                ...
                                return (
                                    <tr key={dept.department} style={{ borderBottom: `1px solid ${t.bg}` }}>
                                        <td style={{ padding: '0.5rem', color: t.text, fontSize: '0.8rem', fontWeight: 500 }}>
                                            {dept.department}
                                        </td>
                                        <td style={{ padding: '0.5rem', color: t.textMuted, fontSize: '0.8rem' }}>
                                            {dept.machine_count}
                                        </td>
                                        <td style={{ padding: '0.5rem', color: t.textMuted, fontSize: '0.8rem' }}>
                                            {dept.event_count}
                                        </td>
                                        <td style={{ padding: '0.5rem', color: dept.blocked_count > 0 ? '#ef4444' : t.textMuted, fontSize: '0.8rem', fontWeight: dept.blocked_count > 0 ? 600 : 400 }}>
                                            {dept.blocked_count}
                                        </td>
                                        <td style={{ padding: '0.5rem', width: '20%' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                <div style={{ flex: 1, height: 6, borderRadius: 3, background: t.bg }}>
                                                    <div style={{
                                                ...
                                                <span style={{ color: t.textMuted, fontSize: '0.75rem', fontWeight: 600, minWidth: 30, textAlign: 'right' }}>
                                                    {blockRate}%
                                                </span>
                                            </div>
                                        </td>
                            ...
                            <tbody>
                                {topMachines.map((item, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <td style={{ padding: '0.45rem 0.5rem', color: t.textFaint, fontSize: '0.75rem', fontWeight: 600, width: 30 }}>
                                                {idx + 1}
                                            </td>
                                            <td style={{ padding: '0.45rem 0.5rem', color: t.text, fontSize: '0.8rem', fontWeight: 500 }}>
                                                {item.machine?.hostname || item.machine_id?.slice(0, 8)}
                                            </td>
                                            <td style={{ padding: '0.45rem 0.5rem', color: t.textMuted, fontSize: '0.75rem' }}>
                                            ...
                                            <td style={{ padding: '0.45rem 0.5rem', width: '35%' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                    ...
                                                    <span style={{ color: t.text, fontSize: '0.75rem', fontWeight: 700, minWidth: 30, textAlign: 'right' }}>
                                                        {item.event_count}
                                                    </span>
                                                </div>
                                            </td>
...
function StatCard({ label, value, color }) {
    ...
    return (
        <div style={{
            ...
            <span style={{ color: t.textMuted, fontSize: '0.7rem', fontWeight: 500 }}>{label}</span>
...
function Legend({ color, label }) {
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
            ...
            <span style={{ color: t.textMuted, fontSize: '0.65rem' }}>{label}</span>
        </div>
    );
}
...
function EmptyState({ text }) {
    const { theme: t } = useTheme();
    return <p style={{ color: t.textFaint, fontSize: '0.8rem', margin: 0 }}>{text}</p>;
}
...
function ExportButton({ label, type, filters }) {
    ...
    return (
        <button
            ...
            style={{
                background: t.border, color: t.textSecondary, border: `1px solid ${t.textSubtle}`,
